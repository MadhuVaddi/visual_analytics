...
//    var bluescale4 = ["#8BA9D0", "#6A90C1", "#066CA9", "#004B8C"];
    var bluescale4 = ["#CC00FF", "#0400ff", "#009BFF", "#00d4ff", "#00FFB4", "#00ff10", "#d7ff00", "#FFEB00", "#FFa000", "#FF5a00", "#FF1e00", "#FF0040"];
...
//defines a function to be used to append the title to the tooltip.  you can set how you want it to display here.
    var maketip = function (d) {
        var tip = '<p class="tip3">' + d.name + '<p class="tip1">Avg. Temp: ' + NumbType(d.value) + '</p> <p class="tip3">' + d.date + '</p>';
        return tip;
    }
...
//create an SVG
    var svg = d3.select("#worldlineId").append("svg")
...
//suck in the data, store it in a value called formatted, run the redraw function
    d3.csv("../data/tempByCountriesLines.csv", function (data) {
        formatted = data;
        redraw();
    });
        ...
        var linedata = color.domain().map(function (name) {
            return {name: name,
                values: data.map(function (d) {
                    return {name: name, date: parseInt(d.date), value: parseFloat(d[name], 10)};
                })
            };
        });
